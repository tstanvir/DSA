...
int dx4[] = {0, 0, 1, -1};
int dy4[] = {1, -1, 0, 0};
int knx[8] = { 2, 1, -1, -2, -2, -1, 1, 2 }; //knight's move in x direction
int kny[8] = { 1, 2, 2, 1, -1, -2, -2, -1 }; //knight's move in y direction 
...
int cs=0,n,k,a;
vi isSp(maxx),edg[maxx],d(maxx),spSubT(maxx),lastA,ans(maxx),ans1(maxx);
void init(){
    rep(i,n+1) isSp[i]=0,edg[i].clear(),d[i]=0,spSubT[i]=0;
    lastA.clear();
}
int dfs(int u,int p){
    if(u!=a) d[u]=d[p]+1;
    if(isSp[u]) spSubT[u]=u;
    forch(v,edg[u]){
        if(v==p) continue;
        int sp=dfs(v,u);
        if(sp and spSubT[u]==0) spSubT[u]=sp;
    }
    return spSubT[u];
}
void dfs1(int u,int p){
    if(spSubT[u]) lastA.pb(u);
    ans[u]=2*d[lastA.back()]-d[u];
    ans1[u]=spSubT[lastA.back()];
    forch(v,edg[u]){
        if(v==p) continue;
        dfs1(v,u);
    }
    if(spSubT[u]) lastA.ppb();
}
void solve(){
    //cout<<"Case "<<++cs<<": ";
    cin>>n>>k>>a;

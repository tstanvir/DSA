...
double dp[103],arr[103];
int n;
/*
The expectation at i is E(i) =
(E(i+1) + E(i+2) + E(i+3) + E(i+4) + E(i+5) + E(i+6)+cnt*gold[i]) /cnt
...
double sol(int pos){
	...
	rep1(i,1,6){
		if(pos+i<n){
			res+=(arr[pos]+sol(pos+i));
		}
		else cnt++;//cnt is invalid position
	}
	res/=(6.0-cnt);//divided by number of valid position
	return dp[pos]=res;
}
void solve(){

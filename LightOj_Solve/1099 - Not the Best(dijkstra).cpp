...
void solve(){
    int n,m;
    scanf("%d%d",&n,&m);
    vi dist1(n+1,(int)1e8),dist2(n+1,(int)1e8),vis(n+1);
    vpii edg[n+1];
    rep(i,m){
        int u,v,w;
        scanf("%d%d%d",&u,&v,&w);
    ...
    while(!q.empty()){
        ...
        forch(it,edg[u]){
            ...
            if(w+cost<dist1[v] and w+cost<dist2[v]){
                dist2[v]=dist1[v];
                dist1[v]=w+cost;
                q.insert({dist1[v],v});
                //q.insert({dist2[v],v});
            }
            else if(w+cost>dist1[v] and w+cost<dist2[v]){
                dist2[v]=w+cost;
                q.insert({dist2[v],v});
            }
            //debug(dist1[v],dist2[v]);
        }
    }
    if(dist2[n-1]<(int)1e8) printf("%d\n",dist2[n-1]);
}

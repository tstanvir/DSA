...
void solve(){
    ...
    s--,t--;
    vpii edg[n],redg[n];
    vi dist1(n,highest(int)),dist2(n,highest(int)),vis(n);
    ...
    forch(it,edgs){
        ...
        if(dist1[u]==highest(int) or dist2[v]==highest(int)) continue;
        int tot=dist1[u]+w+dist2[v];
        if(tot<=p){
            ans=max(ans,w);
        }
    }

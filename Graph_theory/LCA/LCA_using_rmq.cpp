...
void solve(){
    ...
    int rt=-1;
    rep(i,n) if(par[i]==-1) rt=i;
    int timer=0;
    vi eular(2*n+5),st(n+5,-1),ht(n+5),height(2*n+5);
    function<void(int,int)>dfs=[&](int v,int p){
            eular[++timer]=v;
            if(st[v]==-1) st[v]=timer;
            ht[v]=ht[p]+1;
            height[timer]=ht[v];
            forch(it,edg[v]){
                if(it!=p) {
    ...
    rmq rq;
    rq.init(timer+1,height);
    rq.prepo();


    function<int(int,int)>lca=[&](int l,int r){
            return eular[rq.minQ(l,r)]+1;
    };

...
/*
pseudo_code from shafayet's blog:
...
vi edg[maxx],rev[maxx],vis(maxx),color(maxx),components[maxx];
stack<int>stk;
int n,m,mark;
void dfs(int u){
    color[u]=1;
    ...
    stk.push(u);//bigger finishing time comes first
}
void dfs2(int u){
    // dividing the graph into its strongly connected components.
...
void revers(){
    //vital part in the partitioning process. reversing the graph.
    rep1(i,1,n){
        forch(it,edg[i]){
            rev[it].pb(i);
        }
...
void findscc(){
    rep1(i,1,n){//arranging the nodes based on their finishing time.
        if(color[i]==0){
            color[i]=1;
            dfs(i);
        }

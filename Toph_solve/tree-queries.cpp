...
// Contest: Toph
// URL: https://toph.co/p/tree-queries
// Memory Limit: 512 MB
// Time Limit: 2000 ms
// 
...
int cs=0,timer;
int n,q;
vi edg[maxx],arr(maxx),st(maxx),en(maxx),baseArray(maxx*2),subT(maxx);
int dfs(int v,int p){
    st[v]=++timer;
    baseArray[st[v]]=-1;
    int szz=1;
...
void solve(){
    ...
    vi points(arr.begin()+1,arr.begin()+n+1);
    rep(i,q){
    	int u,x;
    	cin>>u>>x;
    	Query[i]={x,st[u],en[u],i,u};
    	points.pb(x);
    }
    ...
    dfs(1,-1);
    rep(i,q){
    	Query[i].x=mm[Query[i].x];
    	Query[i].l=st[Query[i].u];
    	Query[i].r=en[Query[i].u];
    }
    
    vpii vec;
    rep1(i,1,timer){
    	vec.pb({baseArray[i],i});
    }
    ...
    int cur=0;
    rep(i,q){
    	//debug(Query[i].x,vec[cur].ff,vec[cur].ss,Query[i].u);
    	while(vec[cur].ff>=Query[i].x and cur<timer){
    	//	debug("ok");
    		update(vec[cur].ss,1,timer);
    		cur++;
    	}
    	int l=Query[i].l,r=Query[i].r;
    	//debug(l,r);
    	ans[Query[i].ind]=query(r)-query(l-1);
    }
    rep(i,q){
    	cout<<ans[i]<<endl;
    }
